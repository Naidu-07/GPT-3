import React from 'react';

export default function What() {
  return (
    <div className='py-20 px-10'>
      <div className='ammu'>
        <div className='flex justify-between gap-96'>
            <p className='font-bold'>WhatisGPT3</p>
            <p className='text-sky-600 font-medium'>
              We so opinion friends me message as delight. Whole front of plate heard oh ought. His defective nor convinced residence own. Connection has put impossible own apartments boisterous. At jointure ladyship an insisted so humanity he. Friendly bachelor entrance to on.
            </p>
        </div>
        <div className='flex justify-between py-8 px-10'>
          <p className='content text-4xl font-semibold'>
            The possibilities are beyond <br/> your imagination
          </p>
          <p className='text-orange-400 font-medium'>
            Explore the Library
          </p>
        </div>
        <div className='flex py-10 gap-20'>
          <div>
            <p className='font-bold text-lg'>Chatbots </p>
            <p className='text-sky-600 '>We so opinion friends me message as delight. Whole front do of plate heard oh ought.</p>
          </div>
          <div>
            <p className='font-bold text-lg'>Knowledgebase</p>
            <p className='text-sky-600 '> At jointure ladyship an insisted so humanity he. Friendly bachelor entrance to on by. As put impossible apartmentsb.</p>
          </div>
          <div>
            <p className='font-bold text-lg'>Education</p>
            <p className='text-sky-600 '>At jointure ladyship an insisted so humanity he. Friendly bachelor entrance to on by. As put impossible own apartments b</p>
        </div>
      </div>
    </div>
    </div>
  );
}
